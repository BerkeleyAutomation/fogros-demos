...
namespace mpt_ros {
    ...
    class MPTNode {
        ...
        ros::NodeHandle nh_;
        ros::Subscriber envMeshSub_;
        ros::Subscriber robotMeshSub_;
        ros::Subscriber boundsSub_;
        ros::Subscriber planStartToGoalSub_;
        ros::Subscriber motionPlanRequestSub_;
        ros::Publisher motionPlanPub_;
            ...
            double minSolutionCost)
        {
            ...
            ROS_INFO("Planner ran for %lf seconds", Seconds(elapsed).count());
        ...
        auto meshCallback(const std::string& type, const std_msgs::UInt8MultiArray& msg, bool shiftToCenter) {
            ROS_INFO("Got %s, %d, %d, %s", type.c_str(), (int)msg.data.size(), (int)msg.layout.dim.size(),
            ...
            ROS_INFO("Loaded mesh in %lf seconds, tris=%d, verts=%d",
        ...
        static Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> loadMatrix(
            const std::string& name, const std_msgs::Float64MultiArray& msg) 
        {
            int rows = 0;
            int cols = 0;
            ...
            ROS_INFO("Got %s, %dx%d=%d", name.c_str(), rows, cols, (int)msg.data.size());
        ...
        void environmentMeshCallback(const std_msgs::UInt8MultiArray& msg) {
            motionPlanRequest_.environment_ = meshCallback("environment", msg, false);
            optRunPlanner();
        }

        void robotMeshCallback(const std_msgs::UInt8MultiArray& msg) {
            motionPlanRequest_.robot_ = meshCallback("robot", msg, true);
            optRunPlanner();
        }

        void environmentBoundsCallback(const std_msgs::Float64MultiArray& msg) {
        ...
        void planStartToGoalCallback(const std_msgs::Float64MultiArray& msg) {
        ...
        void motionPlanRequestCallback(const mpt_ros::MotionPlanRequest& msg) {
            for (int i=0 ; i<3 ; ++i)
    ...
    public:
        ...
        MPTNode()
            : envMeshSub_(nh_.subscribe("environment_mesh", 1000, &MPTNode::environmentMeshCallback, this))
            , robotMeshSub_(nh_.subscribe("robot_mesh", 1000, &MPTNode::robotMeshCallback, this))
            , boundsSub_(nh_.subscribe("environment_bounds", 1000, &MPTNode::environmentBoundsCallback, this))
            , planStartToGoalSub_(nh_.subscribe("plan_start_to_goal", 1000, &MPTNode::planStartToGoalCallback, this))
            , motionPlanRequestSub_(nh_.subscribe("motion_plan_request", 1000, &MPTNode::motionPlanRequestCallback, this))
            , motionPlanPub_(nh_.advertise<std_msgs::Float64MultiArray>("motion_plan", 1000))
